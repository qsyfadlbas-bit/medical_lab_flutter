...
class LabAppModel extends ChangeNotifier {
  ...
  // ✅ حذف الحساب نهائياً (متطلب سياسة Google Play لحذف الحسابات)
  // ترجع null عند النجاح، أو نص رسالة الخطأ عند الفشل
  Future<String?> deleteAccount(String password) async {
    try {
      final response = await _apiService
          .post('/auth/delete-account', {'password': password});
      final body = ApiService.safeJsonDecode(response);

      if (response.statusCode == 200 &&
          body != null &&
          body['success'] == true) {
        await logout(); // تنظيف كل البيانات المحلية
        return null; // نجاح
      }
  ...
  // ✅ المساعد الذكي صار عبر الخادم (لا يوجد مفتاح داخل التطبيق إطلاقاً)
  //    المنطق الكامل (كاش/Gemini/بديل أوفلاين) داخل AiService.
  ...
  // ✅ 3. الاتصال بالمساعد الذكي عبر الخادم (3 طبقات) مع بديل محلي أوفلاين.
  //    لا مفتاح هنا، ولا يرمي استثناءً — AiService تتكفّل بكل الحالات.
  Future<Map<String, dynamic>> _getRealAIResponse(String question) async {
    final catalogue = _allTests.map((t) => t.toJson()).toList();
    return _aiService.suggest(question, catalogue: catalogue);
  }
...
class _ProfileScreenState extends State<ProfileScreen> {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // ✅ تعديل البيانات الشخصية (الاسم / الهاتف / العنوان)
            _buildSettingsTile(
              icon: Icons.edit_location_alt,
              color: Colors.blue,
              title: "تعديل بياناتي",
              subtitle: "تغيير الاسم أو رقم الهاتف أو العنوان",
              isDark: isDark,
              onTap: () {
            ...
            // ✅ زر حذف الحساب (متطلب Google Play)
            _buildSettingsTile(
              icon: Icons.delete_forever,
              color: Colors.red.shade900,
              title: "حذف الحساب نهائياً",
              subtitle: "حذف حسابك وجميع بياناتك من النظام",
              isDark: isDark,
              onTap: () {
                _showDeleteAccountDialog(context, appModel);
              },
            ),
            ...
            Center(
              child: Column(
                children: [
                  ...
                  // ✅ التعديل هنا: تحويل الـ Container إلى InkWell لجعله قابل للضغط
                  InkWell(
                    onTap: () async {
                      // رابط موقع تيم القادة
                      final Uri url = Uri.parse('https://alqadateam.com/');
                    ...
                    child: Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          const SizedBox(height: 5),
                          // ✅ إضافة النص الجديد الخاص بالرابط
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: const [
                              ...
                              SizedBox(width: 5),
                              Text(
                                "انقر هنا لمعرفة المزيد عن موقعنا",
                                style: TextStyle(
                                  fontFamily: 'Cairo',
                                  fontSize: 12,
                                  color: Colors.blue,
  ...
  // ✅ نافذة تعديل البيانات الشخصية (الاسم / الهاتف / العنوان)
  void _showEditProfileDialog(BuildContext context, LabAppModel appModel) {
    final nameController = TextEditingController(text: appModel.userName);
    final phoneController = TextEditingController(text: appModel.userPhone);
    final addressController = TextEditingController(text: appModel.userAddress);
    bool isSaving = false;
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: 'العنوان',
                    hintText: 'مثال: البصرة - الهارثة - حي الجمهورية',
                    hintStyle:
                  ...
                  : () async {
                      ...
                      if (name.isEmpty || address.isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('الاسم والعنوان مطلوبان',
                      ...
                      final success = await appModel.updateProfile(
                        name: name,
                        phone: phone,
                        email: appModel.userEmail, // البريد يبقى كما هو
                        address: address,
                      );
                        ...
                        SnackBar(
                          content: Text(
                              success
                                  ...
                                  : 'تعذّر الحفظ. تحقق من الإنترنت وحاول مجدداً.',
                              style: const TextStyle(fontFamily: 'Cairo')),
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Text(
                "سيتم حذف حسابك وجميع طلباتك نهائياً ولا يمكن التراجع عن هذا الإجراء.\n\nلتأكيد الحذف، أدخل كلمة المرور:",
            ...
            ElevatedButton(
              ...
              onPressed: isDeleting
                  ...
                  : () async {
                      ...
                      final error = await appModel.deleteAccount(password);

                      if (!ctx.mounted) return;

                      if (error == null) {
                        // ✅ نجح الحذف — العودة لشاشة الدخول
                        Navigator.pop(ctx);
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                  'تم حذف حسابك بنجاح. نتمنى لك السلامة 🌿',
                                  style: TextStyle(fontFamily: 'Cairo')),
                ...
                onPressed: !appModel.isSubmitting
                    ? () async {
                        ...
                        if (success) {
                          ...
                          // ✅ كان الفشل صامتاً — الآن يظهر للمستخدم سبب واضح
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                  "تعذّر إرسال الطلب. تحقق من الإنترنت وحاول مرة أخرى.",
                                  style: TextStyle(fontFamily: 'Cairo')),
                              backgroundColor: Colors.red,
                              behavior: SnackBarBehavior.floating,
                            ),
...
class _HomeScreenState extends State<HomeScreen>
  ...
  // ✅ رأس المساعد — نسخة مرتّبة ومتوازنة (مع التنبيه الطبي لجوجل بلاي)
  Widget _buildAssistantHeader() {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.only(top: 36, left: 24, right: 24, bottom: 30),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            LabTheme.primaryColor,
            LabTheme.secondaryColor.withOpacity(0.92),
          ],
        ),
        borderRadius: const BorderRadius.only(
        ...
        boxShadow: [
          BoxShadow(
            color: LabTheme.primaryColor.withOpacity(0.30),
            blurRadius: 24,
            spreadRadius: 1,
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'صِف أعراضك وسأقترح الفحوصات المناسبة لك',
          ...
          const SizedBox(height: 12),
          // ✅ التنبيه الطبي (مهم جداً لسياسات جوجل بلاي)
          Container(
            ...
            child: Row(
              ...
              children: const [
                ...
                Expanded(
                  child: Text(
                    'تنبيه: هذا المساعد لا يغني عن استشارة الطبيب للتشخيص أو العلاج.',
                    style: TextStyle(
                      fontSize: 10,
                      color: Colors.white,
                      fontFamily: 'Cairo',
  ...
  // ✅ رقاقة مثال قابلة للنقر — تبدأ المحادثة فوراً بالاستعلام المناسب
  Widget _buildExampleChip(String emoji, String label, String query,
      LabAppModel appModel, bool isDark) {
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(22),
            border: Border.all(
              color: LabTheme.primaryColor.withOpacity(0.25),
              width: 1.4,
            ),
            boxShadow: [
              BoxShadow(
                color: LabTheme.primaryColor.withOpacity(0.06),
                blurRadius: 8,
                offset: const Offset(0, 3),
              ),
            ],
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                label,
                style: TextStyle(
                  color: isDark ? Colors.white : LabTheme.primaryColor,
                  fontSize: 13.5,
                  fontWeight: FontWeight.w600,
                  fontFamily: 'Cairo',
                ),
  ...
  // ✅ الحالة الفارغة — مرتبة: رأس + بطاقة إرشاد + أمثلة قابلة للنقر
  Widget _buildCreativeEmptyChat(LabAppModel appModel, bool isDark) {
    final suggestions = <Map<String, String>>[
      {
        'emoji': '💊',
        'label': 'آلام المعدة',
        'query': 'عندي آلام في المعدة وحموضة وحرقة'
      },
      {
        'emoji': '😴',
        'label': 'تعب وإرهاق',
        'query': 'أشعر بتعب وإرهاق مستمر وضعف'
      },
      {
        'emoji': '💓',
        'label': 'ألم في الصدر',
        'query': 'عندي ألم في الصدر وخفقان'
      },
      {
        'emoji': '🩸',
        'label': 'فحص دوري',
        'query': 'أريد إجراء فحوصات دورية للاطمئنان على صحتي'
      },
      {
        'emoji': '🤒',
        'label': 'حرارة وصداع',
        'query': 'عندي ارتفاع حرارة وصداع'
      },
      {
        'emoji': '🦴',
        'label': 'آلام مفاصل',
        'query': 'أعاني من آلام وتورم في المفاصل'
      },
    ];
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: isDark
                  ...
                  : LabTheme.primaryColor.withOpacity(0.06),
              borderRadius: BorderRadius.circular(18),
              border:
                  Border.all(color: LabTheme.primaryColor.withOpacity(0.15)),
            ),
            child: Row(
              children: [
                Container(
                  padding: const EdgeInsets.all(10),
                  decoration: BoxDecoration(
                    color: LabTheme.primaryColor.withOpacity(0.12),
                    shape: BoxShape.circle,
                  ),
                  child: const Icon(Icons.lightbulb_outline,
                      color: LabTheme.primaryColor, size: 22),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Text(
                    'اكتب أعراضك في الأسفل، أو اختر أحد الأمثلة، وسأقترح لك الفحوصات المناسبة.',
                    style: TextStyle(
                      fontSize: 13,
                      height: 1.7,
          ...
          // رقائق منظّمة بالكامل (Wrap) — لا قصّ ولا تمرير أفقي
          Padding(
