...
class AuthProvider with ChangeNotifier {
  ...
  Future<String?> tryAutoLogin() async {
    ...
    // ✅ التحقق الفعلي من صلاحية التوكن عبر السيرفر
    // بدون هذا: توكن منتهي (بعد 30 يوم) = شاشات فارغة وطلبات فاشلة بصمت
    try {
      final response = await _apiService.get('/auth/profile');

      if (response.statusCode == 401 || response.statusCode == 403) {
        // التوكن منتهي أو غير صالح — تنظيف كامل وإجبار تسجيل دخول جديد
        print("⚠️ التوكن منتهي الصلاحية — إعادة توجيه لتسجيل الدخول");
        await prefs.clear();
        _token = null;
        _currentUser = null;
        notifyListeners();
      ...
      // أكواد أخرى (500/503...): خلل مؤقت بالسيرفر — نكمل بالبيانات المحفوظة
    } catch (e) {
      // لا إنترنت أو مهلة — نسمح بالدخول أوفلاين بالبيانات المحفوظة
      print("⚠️ تعذر التحقق من التوكن (أوفلاين؟): $e");
    }
